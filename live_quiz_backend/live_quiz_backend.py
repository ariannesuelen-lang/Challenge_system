...
def repo_insert_pergunta(dados_pergunta: Dict[str, Any]) -> Dict[str, Any]:
    res = banco.table("perguntas").insert(dados_pergunta).execute()
    return res.data[0]
def repo_get_pergunta(pergunta_id: int) -> Optional[Dict[str, Any]]:
    res = banco.table("perguntas").select("*").eq("id", pergunta_id).single().execute()
    return res.data if res.data else None
def repo_get_perguntas_quiz(quiz_id: int) -> List[Dict[str, Any]]:
    res = banco.table("perguntas").select("*").eq("quiz_id", quiz_id).order("id").execute()
    return res.data
def repo_insert_participacao(dados_participacao: Dict[str, Any]) -> Dict[str, Any]:
    res = banco.table("participacoes").insert(dados_participacao).execute()
    return res.data[0]
def repo_get_participacao(participacao_id: int) -> Optional[Dict[str, Any]]:
    res = banco.table("participacoes").select("*").eq("id", participacao_id).single().execute()
    return res.data if res.data else None
def repo_get_participacao_por_aluno_quiz(aluno_id: int, quiz_id: int) -> Optional[Dict[str, Any]]:
    res = banco.table("participacoes").select("*").eq("aluno_id", aluno_id).eq("quiz_id", quiz_id).single().execute()
    return res.data if res.data else None
def repo_update_pontuacao_participacao(participacao_id: int, nova_pontuacao: int) -> Dict[str, Any]:
    res = banco.table("participacoes").update({"pontuacao": nova_pontuacao}).eq("id", participacao_id).execute()
    return res.data[0]
def repo_insert_resposta(dados_resposta: Dict[str, Any]) -> Dict[str, Any]:
    res = banco.table("respostas_alunos").insert(dados_resposta).execute()
    return res.data[0]
def repo_get_ranking_quiz(quiz_id: int) -> List[Dict[str, Any]]:
    res = banco.table("participacoes")\
        .select("id, pontuacao, finalizou, usuarios(nome)")\
        .eq("quiz_id", quiz_id)\
...
def adicionar_pergunta(quiz_id: int, professor_id: int, texto: str, alternativas: List[str], indice_correto: int) -> Dict[str, Any]:
    permitido, motivo = _verificar_permissao_professor(professor_id)
    if not permitido:
        return {"sucesso": False, "mensagem": f"Acesso negado: {motivo}"}
    ...
    try:
        ...
        if quiz["professor_id"] != professor_id:
            return {"sucesso": False, "mensagem": "Este quiz pertence a outro professor."}
        if quiz["status"] != "pendente":
            return {"sucesso": False, "mensagem": "Não é possível alterar um quiz que já foi iniciado ou finalizado."}
    except Exception as e:
...
def entrar_quiz(aluno_id: int, quiz_id: int) -> Dict[str, Any]:
    ...
    try:
        dados = repo_insert_participacao(nova_participacao)
        return {"sucesso": True, "dados": dados}
    except Exception as e:
...
def responder_pergunta(participacao_id: int, pergunta_id: int, indice_resposta: int) -> Dict[str, Any]:
    try:
        participacao = repo_get_participacao(participacao_id)
    ...
    try:
        dados_resposta = repo_insert_resposta(nova_resposta)
...
def obter_perguntas_quiz(quiz_id: int) -> Dict[str, Any]:
    try:
        perguntas = repo_get_perguntas_quiz(quiz_id)
        # Ocultar o índice correto por segurança
        for p in perguntas:
            p.pop("indice_correto", None)
        return {"sucesso": True, "dados": perguntas}
    except Exception as e:
